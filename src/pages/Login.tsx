...
const Login = () => {
  ...
  const handleDiscord = async () => {
    try {
      ...
      toast.error(err.message || "فشل الاتصال بـ Discord");
    }
  };
      ...
      <ScrollReveal>
        <div className="w-full max-w-md">
          ...
          <div className="rounded-xl border border-border/60 bg-card p-8">
            <h1 className="text-xl font-bold text-foreground text-center">مرحباً بعودتك</h1>
            <p className="text-sm text-muted-foreground text-center mt-1">سجّل دخولك إلى حسابك</p>

            <Button variant="secondary" className="w-full mt-6 gap-2" onClick={handleDiscord}>
            ...
            <p className="text-sm text-muted-foreground text-center mt-6">
              ليس لديك حساب؟{" "}
              <Link to="/signup" className="text-primary hover:underline font-medium">إنشاء حساب</Link>
            </p>
          </div>
        </div>
      </ScrollReveal>
